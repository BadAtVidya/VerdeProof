...
import { connectWallet, detectWallet, createConnectedSession, type ConnectedSession } from "../lib/midnight";
...
export function Console() {
  ...
  const connect = async () => {
    ...
    try { const api = await connectWallet(); setSession(await createConnectedSession(api, "/zk/verdeproof/", privateStatePassword)); setNotice("Connected to the live VerdeProof contract."); }
  ...
  const publish = async () => {
    if (!session) return;
    setBusy(true); setError("");
    try { const evidence = await publishRequirement(session, threshold); await refresh(session); setNotice(`Requirement ${short(evidence.requirementId)} finalized in tx ${short(evidence.transactionHash)}`); }
    catch (e) { setError(e instanceof Error ? e.message : String(e)); }
    finally { setBusy(false); }
  };
